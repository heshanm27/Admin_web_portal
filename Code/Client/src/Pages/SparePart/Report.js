...
import SparepartReportpopUpTable from "../../component/Table/sparepartReportTable";
...
async function fetchClaims() {
  const res = await publicRequest.get(`SparePartReport/getAll`);
  return res;
}

const SparePartReportTable = () => {
  ...
  const deleteClaims = (id) => {
    console.log(id);
    return publicRequest.delete(`SparePartReport/delete/${id}`);
  };
  ...
  const deleteReq = useMutation(deleteClaims, {
    onSuccess: () => {
      ...
      queryClient.invalidateQueries("SparePartReport");
    },
    onError: (error) => {
      setNotify({
        isOpen: true,
      ...
      <Container component="main" maxWidth="xl" className={classes.main}>
        ...
        >
          ...
          <Container
            ...
            ref={componentRef}
          >
            <SparepartReportpopUpTable props={params} />
          </Container>
          <div
            style={{
              display: "flex",
...
export default SparePartReportTable;
