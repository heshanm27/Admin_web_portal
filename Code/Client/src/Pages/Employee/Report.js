...
import EmployeeReportPopUptable from "../../component/Table/EmployeeReportTable";
...
async function fetchClaims() {
  const res = await publicRequest.get(`EmployeeReport/getAll`);
  return res;
}
...
const EmployeeReportTable = () => {
  ...
  const deleteClaims = (id) => {
    console.log(id);
    return publicRequest.delete(`EmployeeReport/delete/${id}`);
  };
  ...
  const deleteReq = useMutation(deleteClaims, {
    onSuccess: () => {
      ...
      queryClient.invalidateQueries("EmployeeReport");
    },
    onError: (error) => {
      setNotify({
        isOpen: true,
      ...
      <Container component="main" maxWidth="xl" className={classes.main}>
        ...
        >
          ...
          <Container
            ...
            ref={componentRef}
          >
            <EmployeeReportPopUptable props={params} />
          </Container>
          <div
            style={{
              display: "flex",
