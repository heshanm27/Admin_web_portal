...
import MaintainReportPopUptable from "../../component/Table/MaintainReportTable";
...
async function fetchClaims() {
  const res = await publicRequest.get(`MaintainReport/getAll`);
  return res;
}

const MaintainReportTable = () => {
  ...
  const deleteClaims = (id) => {
    console.log(id);
    return publicRequest.delete(`MaintainReport/delete/${id}`);
  };
  ...
  const deleteReq = useMutation(deleteClaims, {
    onSuccess: () => {
      ...
      queryClient.invalidateQueries("MaintainReport");
    },
    onError: (error) => {
      setNotify({
        isOpen: true,
      ...
      <Container component="main" maxWidth="xl" className={classes.main}>
        ...
        >
          ...
          <Container
            ...
            ref={componentRef}
          >
            <MaintainReportPopUptable props={params} />
          </Container>
          <div
            style={{
              display: "flex",
...
export default MaintainReportTable;
