...
import FinanceReportPopUptable from "../../component/Table/FinanceReportTable";
...
async function fetchClaims() {
  const res = await publicRequest.get(`BillReport/getAll`);
  return res;
}
...
const FinanceReportTable = () => {
  ...
  const deleteClaims = (id) => {
    console.log(id);
    return publicRequest.delete(`BillReport/delete/${id}`);
  };
  ...
  const deleteReq = useMutation(deleteClaims, {
    onSuccess: () => {
      ...
      queryClient.invalidateQueries("BillReport");
    },
    onError: (error) => {
      setNotify({
        isOpen: true,
      ...
      <Container component="main" maxWidth="xl" className={classes.main}>
        ...
        >
          ...
          <Container
            ...
            ref={componentRef}
          >
            <FinanceReportPopUptable props={params} />
          </Container>
          <div
            style={{
              display: "flex",
